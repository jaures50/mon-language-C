...
#define DELAI_RAFRAICHISSEMENT 25 /* Temps en ms entre chaque mise
...
/* Boucle principale */
Partie 3 : Création de jeux 2D en SDL 380/410
www.openclassrooms.comwhile (continuer)
{
...
Partie 3 : Création de jeux 2D en SDL 381/410
www.openclassrooms.com/* On vérifie que la barre ne dépasse pas la hauteur de
la fenêtre
...
DL_UnlockSurface(ecran); /* On a fini de travailler sur
...
p[0] = (pixel >> 16) & 0xff;
p[1] = (pixel >> 8) & 0xff;
Partie 3 : Création de jeux 2D en SDL 382/410
www.openclassrooms.comp[2] = pixel & 0xff;
} else {
p[0] = pixel & 0xff;
p[1] = (pixel >> 8) & 0xff;
p[2] = (pixel >> 16) & 0xff;
